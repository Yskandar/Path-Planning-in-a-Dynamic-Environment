...
class QuadMDP:
	...
	def findEmptyState(self, depth:int) -> list[QuadMDP]:
		space = []
		if self.divided:
			for index in self.Child:
				space.extend(self.child[index].findEmptyState(depth))
		else:
	...
	def findLargerEqualNeighbor(self, direction):
		...
		elif direction == self.Direction.N: # 4
	...
	def getOptimalPath(self, S:list[QuadMDP], depth:int, initialState:tuple, endState:tuple, ):
		...
		#p_bar = tqdm(range(len(E)))
		k = 0
		while pathList and k < len(E):
